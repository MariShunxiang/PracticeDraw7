...
public class Practice02HsvEvaluatorLayout extends RelativeLayout {
  ...
  private class HsvEvaluator implements TypeEvaluator<Integer> {
    ...
    public Integer evaluate(float fraction, Integer startValue, Integer endValue) {
      ...
      // 把　ARGB 转换成 HSV
      Color.colorToHSV(startValue, starHsv);
      Color.colorToHSV(endValue, endHsv);

      // 计算当前动画完成度(fraction)所对应的颜色值
      if (endHsv[0] - starHsv[0] > 180) {
        endHsv[0] -= 360;
      } else if (endHsv[0] - starHsv[0] < -180) {
        endHsv[0] += 360;
      }
      outHsv[0] = starHsv[0] + (endHsv[0] - starHsv[0]) * fraction;
      if (outHsv[0] > 360) {
        outHsv[0] -= 360;
      } else if (outHsv[0] < 0) {
        outHsv[0] += 360;
      }
      outHsv[1] = starHsv[1] + (endHsv[1] - starHsv[1]) * fraction;
      outHsv[2] = starHsv[2] + (endHsv[2] - starHsv[2]) * fraction;

      // 计算动画完成度(fraction)所对应的透明度
